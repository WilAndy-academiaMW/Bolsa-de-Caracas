...
# 1. Importamos la función del JSON
from horario import registrar_en_json 
        ...
        if guardar:
            if os.path.exists(ruta_archivo):
                try:
                    ...
                    # --- CAMBIO AQUÍ: EVITAR REPETIR FECHA ---
                    # Si la fecha ya existe en el archivo, actualizamos esa fila
                    if fecha_actual in df_final['fecha'].values:
                        df_final.loc[df_final['fecha'] == fecha_actual, ["precio", "variacion_abs", "monto_efectivo", "hora"]] = [precio_limpio, var_abs_limpia, monto_limpio, hora_bvc]
                    else:
                        # Si es una fecha nueva, la añadimos al final
                        df_nueva = pd.DataFrame([dict_nuevo]).reindex(columns=columnas)
                        df_final = pd.concat([df_final, df_nueva], ignore_index=True)
                except:
                    df_final = pd.DataFrame([dict_nuevo]).reindex(columns=columnas)
            else:
                df_final = pd.DataFrame([dict_nuevo]).reindex(columns=columnas)

            df_final.to_csv(ruta_archivo, index=False, decimal='.')
            
            # 2. Llamamos al registro del JSON
            registrar_en_json(nombre_simbolo, precio_limpio, hora_bvc)
